...
merged <- df_years2.0(2004, 2024) %>% 
  select(country_name, country_code, year, year_fct, sdg_overall, spi_comp, sci_overall, di_score, regime_type_2, regime_type_4, regch_event, income_level, income_level_lab, gini, log_gdppc, population)
...
```{r}
# data for transitioning / transition event countries 
n_change <- merged %>%
  group_by(country_code) %>%
  filter(any(aut_ep == 1 | dem_ep ==1)) %>% #selects all that have either kind of event 
  mutate(dem_ep_country = any(dem_ep == 1)) %>% #column labels countries that experienced democratization event
  mutate(aut_ep_country = any(aut_ep == 1)) %>% #column labels countries that experienced autocratization event 
  ungroup()

# how many countries experienced either kind of event 
length(n_change$aut_ep_country)

#variables 
n_change_vars <- data.frame(n_change$sdg_overall, n_change$spi_comp, n_change$sci_overall, n_change$di_score)

#how many observations: country-years
colSums(!is.na(n_change_vars))
# how many na's?
colSums(is.na(n_change_vars))
