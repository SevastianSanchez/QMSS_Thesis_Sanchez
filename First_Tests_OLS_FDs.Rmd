...
by_income_class <- ggplot(merged_inc, aes(x = spi_comp, y = sdg_overall, color = income_level)) +
  geom_smooth(data = merged_inc, aes(x = spi_comp, y = sdg_overall, color = NULL), #adding overall regression
              method = "lm", linewidth = 0.75, se = FALSE, color = "black") + #linetype = "dashed"
  # Add income-specific points and regression lines
  ...
  scale_color_manual(values = c("H" = "#1D6A96", 
                    ...
                    labels = c("High Income", 
                              ...
                              "Lower-Middle Income", 
                              "Low Income")) +
  # Add a label for the overall line in the legend
  annotate("text", x = min(merged_inc$spi_comp, na.rm = TRUE), 
           y = max(merged_inc$sdg_overall, na.rm = TRUE), 
           label = "Overall trend", hjust = 0, vjust = 1) +
