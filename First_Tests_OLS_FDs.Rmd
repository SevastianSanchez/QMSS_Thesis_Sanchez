...
sci <- read_csv("~/Desktop/OneDrive/OneDrive/QMSS Thesis/Datasets/SCI_All_Dim_TS.csv")
...
merged <- merge(sdg, spi, by = c("country_code", "year")) 
merged <- merge(merged, sci_scores, by = c("country_code", "year"))
merged <- merge(merged, vdem, by = c("country_code", "year"))
...
## 2019 data only - Cross-Sectional Analysis 
```{r}
...
```{r}
#correlation 
cor(merged$sdg_overall, merged$spi_comp)

#OLS lm 
lm1 <- lm(sdg_overall ~ spi_comp, data = merged)

#residules
plot(lm1)

#scatterplot 
plot(merged$sdg_overall, merged$spi_comp)
