...
controls <- merged %>% select(country_code, year, spi_comp, gdp, gdp_pc, population)
...
ols_spi <- lm(y_var_sdg ~ x_var_spi + merged$gdp_pc + merged$population)
summary(ols_spi)

# 2. OLS for SCI and SDG - Overall 
ols_sci <- lm(y_var_sdg ~ x_var_sci + merged$gdp_pc + merged$population)
summary(ols_sci)

# 3. Multiple Regression with both SPI and SCI
ols_multiple <- lm(y_var_sdg ~ x_var_spi + x_var_sci + merged$gdp_pc + merged$population)
...
SPI reveals a better fit than SCI (0.68 > 0.6585)
...
# put log(GDP), check if pop is relevant, natural log (ln(GDP))
# take best regression in R squared and try with variables and ensure their relevance, 
# check to see if Open data is better, a proxy of SPI _ recommends for missing data proxy despite SPI having it.  
# also look into SCI 
# take few countries that are undergoing regime change 
# describe the limitations, specifically the time series 
