...
overall_lm <- lm(sdg_overall ~ spi_comp + log(gdppc) + population,
                        data = merged)
summary(overall_lm)

# 2. High income countries
high_inc_lm <- lm(sdg_overall ~ spi_comp + log(gdppc) + population,
                        data = merged %>% 
                          filter(income_level == "H"))
summary(high_inc_lm)

# 3. Upper-middle income countries
upper_mid_lm <- lm(sdg_overall ~ spi_comp + log(gdppc) + population,
                         data = merged %>% 
                           filter(income_level == "UM"))
summary(upper_mid_lm)

# 4. Lower-middle income countries
lower_mid_lm <- lm(sdg_overall ~ spi_comp + log(gdppc) + population,
                         data = merged %>% 
                           filter(income_level == "LM"))
summary(lower_mid_lm)

# 5. Low income countries
low_inc_lm <- lm(sdg_overall ~ spi_comp + log(gdppc) + population,
                       data = merged %>% 
...
mod_compare_tab <- data.frame(
  ...
  Std_error = c(
  summary(overall_lm)$coefficients[, 'Std. Error'], # just standard errors 
  summary(high_inc_lm)$coefficients[, 'Std. Error'], # just standard errors 
  summary(upper_mid_lm)$coefficients[, 'Std. Error'], # just standard errors 
  summary(lower_mid_lm)$coefficients[, 'Std. Error'], # just standard errors 
  summary(low_inc_lm)$coefficients[, 'Std. Error'] # just standard errors 
  ),
...
## Vizualizing slopes: changing variable type and labels 
```{r}
# Convert income_level to factor with explicit labels
merged$income_level <- factor(merged$income_level,
                                 levels = c("H", "UM", "LM", "L"),
...
viz_gni_class <- ggplot(data = merged, aes(x = spi_comp, y = sdg_overall, color = income_level)) +
...
viz_gni_class <- viz_gni_class + geom_smooth(data = merged, aes(x = spi_comp, y = sdg_overall), method = "lm",
              linewidth = 0.75, se = FALSE, color = "black")

# Applying theme and labels 
viz_gni_class <- viz_gni_class + 
...
lm_sdg <- lm(sdg_overall ~ regime_type, data = merged_inc)
summary(lm_sdg)

#Mediator model: Check if regime type affects SPI
lm_spi <- lm(spi_comp ~ regime_type, data = merged_inc)
summary(lm_spi)

#outcome model: Check if SPI affects SDG scores while controlling for regime type
lm_sdg_controlled <- lm(sdg_overall ~ spi_comp + regime_type, data = merged_inc)
summary(lm_sdg_controlled)

#Mediation test: Quantify how much of regime type’s effect on SDGs operates through SPI
med_model <- mediate(lm_spi, lm_sdg_controlled, treat = "regime_type", mediator = "spi_comp")
summary(med_model)


lm_spi <- lm(spi_comp ~ regime_type + gdp_pc + population, data = merged_inc)
summary(lm_spi)

lm_reg

med_spi_model <- mediate(model.m = lm_spi, model.y = lm_sdg, treat = "regime_type", mediator = "spi_comp")
summary(med_model)
