...
fd_fe_data <- df_years(yr1=2015) %>%
  dplyr::select(country_code, year, year_fct, sdg_overall, spi_comp, di_score, aut_ep, income_level, income_level_lab, gini, gdppc, log_gdppc, population)
...
##PART 2: Run FD models SDG ~ SPI for each income group
...
## PART 3: Create a comparison table of FD: SDG ~ SPI
...
```{r}
fd_model <- plm(
  spi_comp ~ di_score + log_gdppc + population + year_fct + gini,
  data = fd_fe_data,
