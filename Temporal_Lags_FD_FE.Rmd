...
# First Difference
FD removes all time-invariant characteristics of each unit (like geography, culture, or baseline wealth). However, variables that do change over time, such as GDP per capita and country population, should be controlled.

## FD: SDG ~ SPI 
```{r}
fd_sdg_spi <- plm(
  sdg_overall ~ spi_comp + log_gdppc + population + gini,
  data = fd_fe_data,
...
## FD: SPI ~ DI 
- no need to group by regime type bc not significant interaction
```{r}

fd_spi_di <- plm(
  spi_comp ~ di_score + log_gdppc + population + gini,
  data = fd_fe_data,
  index = c("country_code", "year"),
  model = "fd"
)
...
```{r}
fd_sdg_di <- plm(
  sdg_overall ~ di_score + log_gdppc + population + gini,
  data = fd_fe_data,
  index = c("country_code", "year"),
  model = "fd"
)
...
- ACF checks for independence of errors in regression analysis. This assesses whether x variables are correlated across time (autocorrelation); 
```{r}
# determines correlation at different lags 
...
# FE w/ lags 
fe_sdg_spi_lag <- plm(
  spi_comp ~ di_score + log_gdppc + population + gini + year_fct,
  data = fd_fe_data,
  index = c("country_code", "year"),
  model = "within"
)
...
# FE w/ lags 
fe_sdg_di_lag <- plm(
  sdg_overall ~ di_score + log_gdppc + population + gini + year_fct,
  data = fd_fe_data,
  index = c("country_code", "year"),
  model = "within"
)
