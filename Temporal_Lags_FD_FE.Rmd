...
```{r}
fd_model <- plm(
  spi_comp ~ aut_ep + log(gdppc) + population + factor(year),
  data = fd_fe_data,
...
med_model <- mediate(
  model.m = lm(spi_comp ~ aut_ep + log(gdppc) + population, data = fd_fe_data),
  model.y = lm(sdg_overall ~ spi_comp + aut_ep + gdppc + population, data = fd_fe_data),
  treat = "aut_ep",
  mediator = "spi_comp"
)
...
fd_data <- fd_fe_data %>%
  dplyr::select(country_name.x, country_code, year, sdg_overall, spi_comp, lib_dem, income_level)
...
fd_data <- fd_data %>%
  ...
  mutate(
    ...
    libdem_lag = dplyr::lag(lib_dem, n = 1),
    
    # Calculate first differences
    fd_sdg = sdg_overall - sdg_lag,
    fd_spi = spi_comp - spi_lag,
    fd_libdem = lib_dem - libdem_lag
  ) %>%
  ungroup()

# To verify the lag creation works correctly
head(fd_data %>% dplyr::select(country_code, year, sdg_overall, sdg_lag, fd_sdg, spi_comp, spi_lag, fd_spi) %>% arrange(country_code, year), 10)
```
