...
```{r}
fd_model <- plm(
  spi_comp ~ aut_ep + log(gdppc) + population + year_fct + gini,
  data = fd_fe_data,
...
med_model <- mediate(
  model.m = lm(spi_comp ~ aut_ep + log(gdppc) + population + gini, data = fd_fe_data),
  model.y = lm(sdg_overall ~ spi_comp + aut_ep + log(gdppc) + population + gini, data = fd_fe_data),
  treat = "aut_ep",
  mediator = "spi_comp"
)
