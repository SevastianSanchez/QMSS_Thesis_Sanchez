...
# how many countries experiences regime transitions?
length(unique(REGIME_CH_INSTANCES$country_code)) # 34 countries experienced regime transitions or

# how many countries democratized? 

# how many countries autocratized?
...
sapply(panel_data_comp3 %>% 
         select(eiu_regime_type, eiu_regch_event, eiu_dem_ep, eiu_aut_ep, eiu_has_aut_ep, eiu_has_dem_ep, eiu_has_both, eiu_has_neither, eiu_democratized, eiu_autocratized, eiu_stable), 
       unique)

# Check counts of regime transitions and episodes
table(panel_data_comp3$eiu_regch_event)
table(panel_data_comp3$eiu_dem_ep)
table(panel_data_comp3$eiu_aut_ep)
table(panel_data_comp3$eiu_has_aut_ep)
table(panel_data_comp3$eiu_has_dem_ep)
table(panel_data_comp3$eiu_has_both)
table(panel_data_comp3$eiu_has_neither)
table(panel_data_comp3$eiu_democratized)
table(panel_data_comp3$eiu_autocratized)
table(panel_data_comp3$eiu_stable)

# Check event time distributions
summary(panel_data_comp3$eiu_dem_event_time)
summary(panel_data_comp3$eiu_aut_event_time)
summary(panel_data_comp3$eiu_dem_ep_time)
summary(panel_data_comp3$eiu_aut_ep_time)
...
table(panel_data_comp3$eiu_regime_type)

# income_level_recoded 
table(panel_data_comp3$income_level_recoded)
```

## filter out transitioning regimes 
```{r}
panel_data <- panel_data_comp3 %>%
  dplyr::filter(di_reg_type_2 != 1) # removing transitioning regimes
