...
panel_data <- panel_data %>% 
  dplyr::select(country_name, country_code, year, spi_comp, di_score, di_score_lag1, di_score_lag2, log_gdppc,
                spi_comp_lag1, spi_comp_lag2, income_level_recoded) %>%
...
head(panel_data[, c("country_code", "year", "spi_comp", "spi_comp_lag1", "spi_comp_lag2",  "di_score", "di_score_lag1", "di_score_lag2")])
...
ebar_fe <- ggplot(coef_df, aes(x = term, y = estimate)) +
  ...
  labs(title = "Stage I: FE Model (SPI ~ DI)", 
       x = NULL, 
...
ebar_fd <- ggplot(coef_fd_df, aes(x = term, y = estimate)) +
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error), width = 0.2) +
  labs(title = "Lagged Effects in FD Model (SPI ~ DI)", x = NULL, y = "Coefficient Estimate") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_x_discrete(labels = c("di_score" = "Democracy (DI)", 
                              "di_score_lag1" = "Lagged DI (t-1)", 
