...
head(panel_data[, c("country_code", "year", "spi_comp_lag1", "di_score_lag1", "log_gdppc_lag1")])
```
...
# Adding Lag2: SPI ~ DI
ols_spi_di_L2 <- plm(
  formula = spi_comp ~ di_score + plm::lag(di_score, n=1) + plm::lag(di_score, n=2) + log_gdppc + income_level_recoded + factor(year), 
  index = c("country_code", "year"),
  model = "pooling", 
  data = panel_data)
...
# Adding Lag1: SPI ~ DI
fd_spi_di_L1 <- plm(
  formula = spi_comp ~ di_score + plm::lag(di_score, n=1) + log_gdppc + income_level_recoded,
  index = c("country_code", "year"),
  data = fd_data, #sub = (has_aut_ep == TRUE),
  model = "fd"
)
...
# Adding Lag2: SPI ~ DI
fd_spi_di_L2 <- plm(
  formula = spi_comp ~ di_score + plm::lag(di_score, n=1) + plm::lag(di_score, n=2) + log_gdppc + income_level_recoded,
  index = c("country_code", "year"),
  data = fd_data, #sub = (has_aut_ep == TRUE),
  model = "fd"
)
...
# Adding Lag2: SPI ~ DI
fe_spi_di_L2 <- plm(
  formula = spi_comp ~ di_score + plm::lag(di_score, n=1) + dplyr::lag(di_score, n=2) + log_gdppc + income_level_recoded,
  index = c("country_code", "year"),
  data = panel_data,
  model = "within", #FE
  effect = "twoways" 
...
# Visualize the results using error bar charts that show the coefficient and robust standard error of every lagged effect in the fe_spi_di_L2 FE model
```{r}
...
# visualize the results using error bar charts that show the coefficient and robust standard error of every lagged effect in the fd_spi_di_L2 FD model
```{r}
