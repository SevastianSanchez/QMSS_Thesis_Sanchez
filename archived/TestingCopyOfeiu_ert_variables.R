...
panel_data_t <- panel_data %>%
  select(country_name, country_code, year, di_score, spi_comp)

  # Validate all columns exist in DF
  required_cols <- c("country_code", "year", "di_score")
  # check for missing required columns
  missing_cols <- setdiff(required_cols, names(panel_data_t)) 
