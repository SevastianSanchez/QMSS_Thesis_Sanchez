...
# FE Model - ALL
fe_testing_all <- plm(
  formula = spi_comp ~ di_score + log_gdppc + aut_ep + factor(year), 
...
```{r}
fd_model <- plm(
  spi_comp ~ di_score + log(gdppc) + year_fct + gini,
  data = merged_testing,
...
```{r}
library(plm)
fe_model <- plm(sdg_overall ~ spi_comp + regime_type + log_gdppc,
                data = merged_testing, 
                index = c("country_code", "year"), 
                model = "within")
...
med_model <- mediate(
  model.m = lm(spi_comp ~ di_score + log(gdppc) + gini, data = fd_fe_data),
  model.y = lm(sdg_overall ~ spi_comp + di_score + log(gdppc) + gini, data = fd_fe_data),
  treat = "aut_ep",
  mediator = "spi_comp"
)
