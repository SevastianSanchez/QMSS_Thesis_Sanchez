...
###### SPI ~ DI (M ~ X) ###### LAG + DVL 
fe_spi_di_lag4 <- plm(
  formula = spi_comp ~ di_score + lag(di_score, 1) + lag(spi_comp, 1) + log_gdppc + factor(year),
  index = c("country_code", "year"),
  data = fd_fe_data,
  model = "within" 
)
...
###### SDG ~ SPI (Y ~ M) ###### 4. LAGS + LDV
fe_sdg_spi_lag4 <- plm(
  sdg_overall ~ lag(spi_comp, 1) + lag(di_score, 1) + lag(sdg_overall, 1) + log_gdppc + factor(year),
  index = c("country_code", "year"),
  data = fd_fe_data,
  effect = "twoways",
  model = "within"
