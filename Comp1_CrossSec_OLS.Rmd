...
### Test 2a: Ramsey RESET Test & Residual Diagnostics for Misspecification
```{r}
...
reset_results <- data.frame(
  model = c("ols_base_lm", "ols_sci_lm", "ols_multiple_lm"),
  F_statistic = c(resettest(ols_base_lm, power = 2:3, type = "regressor", 
                             vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))$statistic,
                  resettest(ols_sci_lm, power = 2:3, type = "regressor",
                             vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))$statistic,
                  resettest(ols_multiple_lm, power = 2:3, type = "regressor",
                             vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))$statistic),
  p_value = c(resettest(ols_base_lm, power = 2:3, type = "regressor", 
...
write.csv(reset_results, file = "output_CSVs/reset_test_results.csv", row.names = FALSE)
```
...
stargazer(ols_spi_reg, ols_spi_quad1, ols_spi_quad2, ols_spi_quad2b, #ols_spi_quad,
          ...
          column.labels = c("Linear", "Quad GDP", "Quad SPI+GDP", "Quad DI+GDP"),
                            #, "Full Quadratic"),
...
write_csv(all_models_tidy, "output_CSVs/interaction_models.csv")
