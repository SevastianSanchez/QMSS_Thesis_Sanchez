...
## Test 2: Non-Linear Functional Form Terms
...
**Procedure:** 
1) Ramsey RESET Test for Misspecification & Omitted Interactions (non-linearity)
2) Check residuals for non-linearity and heteroskedasticity: 
   - Residuals vs Fitted Values plot (for each model)
   - Residuals vs Predictor plot (for each predictor)
3) Establish main effects with pooled model with robust SE
4) Establish non-linear (quadratic, interactions, logged, etc.) model with robust SE (all continuous are quadratic)
5) Reduce the model by dropping non-significant terms (if any)
6) Compare AIC/BIC & Adjusted R-Sq values for both pooled and reduced model
7) Repeat steps 1-6 with Interactions
8) Re-run Ramsey RESET with final model to confirm no misspecification

### Test 2a: Ramsey RESET Test for Misspecification
```{r}
#Transforming to standard OLS for resettest
ols_base_lm <- lm(sdg_overall ~ spi_comp + di_score + log_gdppc + factor(year), data = merged)
ols_sci_lm <- lm(sdg_overall ~ sci_overall + di_score + log_gdppc + factor(year), data = merged)
ols_multiple_lm <- lm(sdg_overall ~ spi_comp + sci_overall + di_score + log_gdppc + factor(year), data = merged)

# Run RESET tests w/ Robust SEs
resettest(ols_base_lm, power = 2:3, type = "regressor", 
          vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))
resettest(ols_sci_lm, power = 2:3, type = "regressor",
          vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))
resettest(ols_multiple_lm, power = 2:3, type = "regressor",
          vcov = function(x) vcovHC(x, cluster = "group", type = "HC1"))
```
The Ramsey RESET test results indicate that the null hypothesis of no omitted variables or non-linear relationships can be rejected for all three models (SPI, SCI, and Multiple). The p-values are all less than 0.05, suggesting that there are indeed non-linear relationships or omitted interactions in the models. This implies that the functional form of the models may not be correctly specified, and further investigation into non-linear terms or interactions is warranted.

### Residuals Plots: Checking for Non-Linearity
```{r}
# Wrangling for analysis and multicollinearity concerns 
merged_2015 <- merged %>% 
  # Filtering to include only data from 2016 onwards
  filter(year >= 2016) %>% 
  # Selecting relevant variables for analysis 
  select(country_name, country_code, year, sdg_overall, spi_comp, di_score, log_gdppc, income_level, aut_ep, dem_ep, regime_type_2, regime_type_4)

# Extract the data actually used in the base model
ols_base_lm <- lm(sdg_overall ~ spi_comp + di_score + log_gdppc + factor(year), data = merged_2015) # regular linear model
model_data <- model.frame(ols_base_lm)

# saving as png 
png("figures/ols_residual_plots.png", width = 8, height = 7, units = "in", res = 300)
par(mfrow = c(2, 2))

# Residuals vs Fitted Values plot for ols_base_lm
plot(ols_base_lm, which = 1,
     main = "Residuals vs Fitted Model", caption = "ols_base_lm", pch = 1, cex = 0.65, col = "#595959")
...
plot(model_data$spi_comp, resid(ols_base_lm),
     xlab = "spi_comp", ylab = "Residuals", main = "Residuals vs spi_comp", pch = 1, cex = 0.65, col = "darkblue")
abline(h = 0, col = "black", lty = 2, lwd = 1)
lines(lowess(model_data$spi_comp, resid(ols_base_lm)), col = "red", lwd =  1.5)

plot(model_data$di_score, resid(ols_base_lm),
     xlab = "di_score", ylab = "Residuals", main = "Residuals vs di_score", pch = 1, cex = 0.65, col = "darkred")
abline(h = 0, col = "black", lty = 2, lwd = 1)
lines(lowess(model_data$di_score, resid(ols_base_lm)), col = "red", lwd = 1.5)

# Residuals vs gdppc plot
plot(model_data$log_gdppc, resid(ols_base_lm),
     xlab = "log_gdppc", ylab = "Residuals", main = "Residuals vs log_gdppc", pch = 1, cex = 0.65, col = "darkgreen")
abline(h = 0, col = "black", lty = 2, lwd = 1)
lines(lowess(model_data$log_gdppc, resid(ols_base_lm)), col = "red", lwd =  1.5)
...
ols_spi_quad <- plm(formula = sdg_overall ~ spi_comp + I(spi_comp^2) + di_score + I(di_score^2) + log_gdppc + I(log_gdppc^2) + factor(year),
               ...
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_quad <- summary(ols_spi_quad, vcov = vcovHC(ols_spi_quad, cluster = "group", type = "HC1"))

#AIC/BIC for quadratic model
source('plm_aic_bic_function.R')
plm_aic_bic(sumry_ols_quad)

# Assess multi-colinearity 
vif(ols_spi_quad)

# Correcting for multicollinearity: centering variables
...
ols_spi_reg <- plm(formula = sdg_overall ~ cen_spi_comp + cen_di_score + cen_log_gdppc + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_reg <- summary(ols_spi_reg, vcov = vcovHC(ols_spi_reg, cluster = "group", type = "HC1"))

# 1 quadratic term
ols_spi_quad1 <- plm(formula = sdg_overall ~ cen_spi_comp + cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_quad1 <- summary(ols_spi_quad1, vcov = vcovHC(ols_spi_quad1, cluster = "group", type = "HC1"))

# 2 quadratic terms
ols_spi_quad2 <- plm(formula = sdg_overall ~ cen_spi_comp + I(cen_spi_comp^2) + cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_quad2 <- summary(ols_spi_quad2, vcov = vcovHC(ols_spi_quad2, cluster = "group", type = "HC1"))

# 2 quadratic terms
ols_spi_quad2b <- plm(formula = sdg_overall ~ cen_spi_comp + cen_di_score + I(cen_di_score^2) + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_quad2b <- summary(ols_spi_quad2b, vcov = vcovHC(ols_spi_quad2b, cluster = "group", type = "HC1"))

# 3 quadratic terms - quadratic model 
ols_spi_quad <- plm(formula = sdg_overall ~ cen_spi_comp + I(cen_spi_comp^2) + cen_di_score + I(cen_di_score^2) + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged_2015)
sumry_ols_quad <- summary(ols_spi_quad, vcov = vcovHC(ols_spi_quad, cluster = "group", type = "HC1"))
...
rob_se_reg    <- coeftest(ols_spi_reg, vcov = vcovHC(ols_spi_reg, cluster = "group", type = "HC1"))
rob_se_quad1  <- coeftest(ols_spi_quad1, vcov = vcovHC(ols_spi_quad1, cluster = "group", type = "HC1"))
rob_se_quad2  <- coeftest(ols_spi_quad2, vcov = vcovHC(ols_spi_quad2, cluster = "group", type = "HC1"))
rob_se_quad2b <- coeftest(ols_spi_quad2b, vcov = vcovHC(ols_spi_quad2b, cluster = "group", type = "HC1"))
rob_se_quad3  <- coeftest(ols_spi_quad, vcov = vcovHC(ols_spi_quad, cluster = "group", type = "HC1"))

# stargazer table
stargazer(ols_spi_reg, ols_spi_quad1, ols_spi_quad2, ols_spi_quad2b, #ols_spi_quad,
          se = list(rob_se_reg[, 2], rob_se_quad1[, 2], rob_se_quad2[, 2], rob_se_quad2b[, 2]#, rob_se_quad3[, 2]
                    ),
          title = "Regression Models with Robust Standard Errors",
          align = TRUE,
          dep.var.labels = "SDG Overall Score",
          column.labels = c("Linear", "Quad GDP", "Quad SPI+GDP", "Quad DI+GDP"#, "Full Quadratic"
                            ),
         covariate.labels = c("SPI Component", "SPI Squared", "Democracy Index", "Democracy Index Squared",
                               "Log GDPpc", "Log GDPpc Squared", "testing", "Intercept"),
          ...
          model.numbers = FALSE,
          keep.stat = c("n", "rsq", "adj.rsq"),
         out = "figures/pooled_ols_models_quadratic.html") #saved as html

# AIC/BIC for both models (sourcing function for plm models)
plm_aic_bic(sumry_ols_reg) # regular pooled ols
plm_aic_bic(sumry_ols_quad1) # quadratic pooled ols
plm_aic_bic(sumry_ols_quad2) # quadratic pooled ols
plm_aic_bic(sumry_ols_quad2b) # quadratic pooled ols
plm_aic_bic(sumry_ols_quad) # quadratic pooled ols
```
The quadratic model shows a meaningful improvement in explanatory power. 

Comparing the linear and quadratic pooled OLS models with robust clustered standard errors, the quadratic specification provides a noticeably better fit for explaining SDG performance. In the linear model, statistical capacity (spi_comp) and log GDP per capita (log_gdppc) are both highly significant predictors, while the democracy index (di_score) is not significant. However, after adding quadratic terms for all three continuous predictors, we uncovered evidence of a non-linear underlying relational structure. All continuous explanatory variables (SPI, DI and Log(GDP Per-Cap)) are statistically significant (as quadratic variables), suggesting a non-linear relationship exists between predictors and SDG performance. The quadratic model shows a meaningful improvement in explanatory power, with the adjusted R-squared improving from 0.7757 to 0.8202 (4.45 percentage point increase in adjusted R²). 

To reiterate, the linear term for democracy index, previously non-significant, becomes significant when its quadratic is included, suggesting a non-linear relationship. The linear term for statistical capacity loses significance, but the quadratic remains, which—according to the hierarchical principle (20XX)—means both should be retained. Overall, these results indicate that all three predictors exhibit important non-linear effects on SDG performance, which upholds the quadratic form as superior. Further analysis does not necessitate any reductions; all included predictors (linear and quadratic) should remain in the model, as each contributes to capturing the complexity of the data.

Further validated by AIC/BIC results, the quadratic model has lower AIC and BIC values compared to the linear model, indicating a better fit. The AIC for the linear model is 7446.06, while the quadratic model's AIC is 7193.846. Similarly, the BIC for the linear model is 7671.324, and for the quadratic model, it is 7480.546.

Although these results show meaningful improvements in the quadratic model, it is important to note that VIF estimates for the quadratic terms are higher than those for the linear terms, signaling multicollinearity issues (Appendix X). The severity of VIF results (all > 10) suggests that quadratic terms are inflating the variance of the coefficient estimates. Assessing which variable(s) is causing the most non-linearity is essential to determine if the quadratic terms are necessary.

### Test 2c: Checking for Interactions
...
merged_2015 <- merged_2015 %>% 
  ...
  mutate(income_level_recoded = case_when(
    income_level == "L" ~ 1, # Low-Income
    income_level == "LM" ~ 2, # Lower-Middle-Income
    income_level == "UM" ~ 3, # Upper-Middle-Income
    income_level == "H" ~ 4, # High-Income
...
spi_x_inc_lvl <- lm(sdg_overall ~ cen_spi_comp*factor(income_level_recoded) + cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year), data = merged_2015)
...
spi_x_reg_binary <- lm(sdg_overall ~ cen_spi_comp*factor(regime_type_binary) + cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year), data = merged_2015)
...
spi_x_reg_categ <- lm(sdg_overall ~ cen_spi_comp*factor(regime_type_categ) + cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year), data = merged_2015)
...
spi_x_di_score <- lm(sdg_overall ~ cen_spi_comp*cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year), data = merged_2015)
summary(spi_x_di_score, vcov = vcovHC(spi_x_di_score, cluster = "group", type = "HC1"))

# Final Model 
final_model <- lm(sdg_overall ~ cen_spi_comp*factor(income_level_recoded) + cen_spi_comp*cen_di_score + cen_log_gdppc + I(cen_log_gdppc^2) + factor(year), data = merged_2015)
summary(final_model, vcov = vcovHC(final_model, cluster = "group", type = "HC1"))

```
*Interaction 1: GNI Income Classification: Yes* All GNI classifications (High, Upper-Middle, Lower-Middle, and Low) exhibit statistically significant interactions affecting the relationship between SPI and SDG performance. But the strength of the interaction varies across income levels, with the strongest interaction found in ___
...
# collect all interaction models into a list for further analysis 
interaction_models <- list(
  spi_x_inc_lvl = spi_x_inc_lvl,
  spi_x_reg_binary = spi_x_reg_binary,
  spi_x_reg_categ = spi_x_reg_categ,
  spi_x_di_score = spi_x_di_score
)
# Extracting robust standard errors for each interaction model
robust_se_interactions <- lapply(interaction_models, function(model) {
  coeftest(model, vcov = vcovHC(model, cluster = "group", type = "HC1"))
})
# Extracting adjusted R-sq and number of observations for each interaction model
interaction_adj_r_squared <- sapply(interaction_models, function(model) {
  summary(model)$adj.r.squared
})
interaction_nobs <- sapply(interaction_models, nobs)
# Create a summary table for interaction models
interaction_summary <- lapply(names(interaction_models), function(model_name) {
  model <- interaction_models[[model_name]]
  coefs <- robust_se_interactions[[model_name]]
  data.frame(
    model = model_name,
    term = rownames(coefs),
...
interaction_df <- bind_rows(interaction_summary)
# Save the interaction_df to a CSV file
write.csv(interaction_df, file = "output_CSVs/interaction_df.csv", row.names = FALSE)
