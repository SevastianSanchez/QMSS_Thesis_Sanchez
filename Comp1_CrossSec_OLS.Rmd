...
## Preliminary Analysis: Correlation & Naive OLS Models
...
R-sq (SPI): 0.616037202309322 [high correlation with SDG score]
R-sq (SCI): 0.410940651230861
R-sq (DI): 0.452968121616442
...
When compared in separate models, SPI has a greater impact on SDG status (0.28735224) than SCI (0.23763344). This suggests that a one-unit increase in SPI is associated with a larger improvement in SDG outcomes compared to a one-unit increase in SCI, holding all controls constant. 
...
```
[Co-pilot] The Ramsey RESET test for model misspecification indicates that all three models (SPI, SCI, and Multiple) show significant evidence of misspecification. This suggests that the models may not adequately capture the relationship between the predictors and the SDG composite scores. This could be due to omitted variables, non-linear relationships, or other forms of model misspecification. The p-values for the RESET test are all less than 0.001, indicating strong evidence against the null hypothesis of no misspecification.This suggests that the models may not adequately capture the relationship between the predictors and the SDG composite scores.

Given the high variablility of statistical capacity measures and control variables like GDP per capita, misspecification is likely to be the result of omitted interaction terms or heteroskedasticity.

To test for the existence of non-linear omitted variables, or lack thereof, I deploy a Breusch-Pagan test for heteroskedasticity.
