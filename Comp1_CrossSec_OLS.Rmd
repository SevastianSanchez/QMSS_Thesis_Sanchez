...
# Compute robust F-statistics (Wald tests)
wald_spi <- waldtest(ols_spi, vcov = vcovHC(ols_spi, type = "HC1"))
wald_sci <- waldtest(ols_sci, vcov = vcovHC(ols_sci, type = "HC1"))
wald_multiple <- waldtest(ols_multiple, vcov = vcovHC(ols_multiple, type = "HC1"))

# Generate the stargazer table with ALL robust statistics
stargazer(ols_spi, ols_sci, ols_multiple,
  ...
  add.lines = list(
    c("Robust F-statistic", 
      sprintf("%.2f", wald_spi$F[2]), 
      sprintf("%.2f", wald_sci$F[2]), 
      sprintf("%.2f", wald_multiple$F[2])),
    c("Robust F p-value", 
      format.pval(wald_spi$`Pr(>F)`[2], digits = 3), 
      format.pval(wald_sci$`Pr(>F)`[2], digits = 3), 
      format.pval(wald_multiple$`Pr(>F)`[2], digits = 3))
  )
  # out = "figures/ols_spi_nonRobust"  # Uncomment to save to file
)
...
**colinearity:** The correlation between SCI and SPI is about 0.8277. When placed within the same model, SCI inflated the standard error of SPI from 0.0216 to 0.03062. SCI had a similar reaction from the SPI with its standard error increasing from 0.01677 to 0.03017.
...
ols_spi           65.9       2.82e-12        
ols_sci           92.1       1.10e-17 
ols_multiple      23.2       1.59e-3 
