...
resettest(ols_spi_lm, power = 2:3, type = "regressor", 
...
ols_reg_lm <- lm(sdg_overall ~ spi_comp + di_score + log_gdppc + factor(year), data = merged) # regular linear model
model_data <- model.frame(ols_reg_lm)

# saving as png 
png("figures/ols_residual_plots.png", width = 8, height = 7, units = "in", res = 300)
par(mfrow = c(2, 2))

# Residuals vs Fitted Values plot for ols_reg_lm
plot(ols_reg_lm, which = 1,
     main = "Residuals vs Fitted Model", caption = "ols_reg_lm", pch = 1, col = "black")
abline(h = 0, col = "red", lty = 2, lwd = 1)
fitted_vals <- fitted(ols_reg_lm)
lines(lowess(fitted_vals, resid(ols_reg_lm)), col = "darkorange", lwd = 2)

# Residuals vs Predictor plots
plot(model_data$spi_comp, resid(ols_reg_lm),
     xlab = "spi_comp", ylab = "Residuals", main = "Residuals vs spi_comp", pch = 1, col = "darkblue")
abline(h = 0, col = "red", lty = 2, lwd = 1)
lines(lowess(model_data$spi_comp, resid(ols_reg_lm)), col = "darkorange", lwd = 2)

plot(model_data$di_score, resid(ols_reg_lm),
     xlab = "di_score", ylab = "Residuals", main = "Residuals vs di_score", pch = 1, col = "darkred")
abline(h = 0, col = "red", lty = 2, lwd = 1)
lines(lowess(model_data$di_score, resid(ols_reg_lm)), col = "darkorange", lwd = 2)

# Residuals vs gdppc plot
plot(model_data$log_gdppc, resid(ols_reg_lm),
     xlab = "log_gdppc", ylab = "Residuals", main = "Residuals vs log_gdppc", pch = 1, col = "darkgreen")
abline(h = 0, col = "red", lty = 2, lwd = 1)
lines(lowess(model_data$log_gdppc, resid(ols_reg_lm)), col = "darkorange", lwd = 2)
...
### Log Transformation: log(gdppc) & log(gdppc)^2 Residuals Plots
```{r}
# Residuals vs log(gdppc) plot
plot(log(model_data$gdp_pc), resid(ols_reg_lm),
     xlab = "log_gdppc", ylab = "Residuals", main = "Residuals vs gdp_pc", pch = 1, col = "darkgreen")
abline(h = 0, col = "red", lty = 2, lwd = 1)
lines(lowess(log(model_data$gdp_pc), resid(ols_reg_lm)), col = "black", lwd = 2)

# Residuals vs log(gdppc)^2 plot
plot(I(log(model_data$gdp_pc)^2), resid(ols_reg_lm),
     xlab = "log_gdppc", ylab = "Residuals", main = "Residuals vs gdp_pc", pch = 16, col = "darkgreen")
abline(h = 0, col = "red", lty = 2, lwd = 1)
lines(lowess(I(log(model_data$gdp_pc)^2), resid(ols_reg_lm)), col = "black", lwd = 2)
...
# 1 quadratic term
ols_spi_quad1 <- plm(formula = sdg_overall ~ spi_comp + di_score + log_gdppc + I(log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged)
sumry_ols_quad1 <- summary(ols_spi_reg1, vcov = vcovHC(ols_spi_quad1, cluster = "group", type = "HC1"))
sumry_ols_quad1

# 2 quadratic terms
ols_spi_quad2 <- plm(formula = sdg_overall ~ spi_comp + I(spi_comp^2) + di_score + log_gdppc+ I(log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged)
...
# 2 quadratic terms
ols_spi_quad2b <- plm(formula = sdg_overall ~ spi_comp + di_score + I(di_score^2) + log_gdppc+ I(log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged)
...
# 3 quadratic terms
ols_spi_quad3 <- plm(formula = sdg_overall ~ spi_comp + I(spi_comp^2) + di_score + I(di_score^2) + log_gdppc + I(log_gdppc^2) + factor(year),
               model = "pooling", 
               index = c("country_code", "year"),
               data = merged)
sumry_ols_quad3 <- summary(ols_spi_quad3, vcov = vcovHC(ols_spi_quad3, cluster = "group", type = "HC1"))
sumry_ols_quad3

# Extracting robust standard errors for each model
rob_se_reg <- sqrt(diag(vcovHC(ols_spi_reg, cluster = "group", type = "HC1")))
rob_se_quad1 <- sqrt(diag(vcovHC(ols_spi_quad1, cluster = "group", type = "HC1")))
rob_se_quad2 <- sqrt(diag(vcovHC(ols_spi_quad2, cluster = "group", type = "HC1")))
rob_se_quad2b <- sqrt(diag(vcovHC(ols_spi_quad2b, cluster = "group", type = "HC1")))
rob_se_quad3 <- sqrt(diag(vcovHC(ols_spi_quad3, cluster = "group", type = "HC1")))

# Stargazer table for all quad models with robust standard errors
stargazer(ols_spi_reg, ols_spi_quad1, ols_spi_quad2, 
          #ols_spi_quad2b, 
          ols_spi_quad3,
          se = list(rob_se_reg, rob_se_quad1, rob_se_quad2,
                    #rob_se_quad2b, 
                    rob_se_quad3),
          title = "Pooled OLS Models with Quadratic Terms",
          align = TRUE,
          dep.var.labels = "SDG Overall Performance",
          # Don't specify covariate.labels initially - let stargazer auto-generate
          column.labels = c("Linear Baseline", "1 Quadratic", "2 Quadratic (A)", 
                            #"2 Quadratic (B)", 
                            "3 Quadratic"),
          keep.stat = c("n", "rsq", "adj.rsq"),
          type = "html",
          out = "figures/pooled_ols_models_quadratic.html")
...
plm_aic_bic(sumry_ols_quad3) # quadratic pooled ols
```
