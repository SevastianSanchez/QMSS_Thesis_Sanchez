...
ols_sdg_spi <- plm(formula = sdg_overall ~ spi_comp + di_score + log_gdppc + income_level_recoded + factor(year), 
               model = "pooling", 
...
# Adding Lag1: SPI ~ DI
ols_sdg_spi_L1 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + di_score + dplyr::lag(di_score, 1) + log_gdppc + income_level_recoded + factor(year), 
               model = "pooling", 
               index = c("country_code", "year"),
               data = panel_data)
...
# Adding Lag2: SPI ~ DI
ols_sdg_spi_L2 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + dplyr::lag(spi_comp, 2) + di_score + dplyr::lag(di_score, 1) + dplyr::lag(di_score, 2) + log_gdppc + income_level_recoded + factor(year), 
               model = "pooling", 
               index = c("country_code", "year"),
               data = panel_data)
...
```{r}
# Contemporaneous Effect: SDG ~ SPI + DI
fd_sdg_spi <- plm(formula = sdg_overall ~ spi_comp + di_score + log_gdppc + income_level_recoded + factor(year), 
               model = "fd", 
               index = c("country_code", "year"),
               data = fd_data)
...
# Adding Lag1: SPI ~ DI
fd_sdg_spi_L1 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + di_score + dplyr::lag(di_score, 1) + log_gdppc + income_level_recoded + factor(year), 
               model = "fd", 
               index = c("country_code", "year"),
               data = fd_data)
...
# Adding Lag2: SPI ~ DI
fd_sdg_spi_L2 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + dplyr::lag(spi_comp, 2) + di_score + dplyr::lag(di_score, 1) + dplyr::lag(di_score, 2) + log_gdppc + income_level_recoded + factor(year), 
               model = "fd", 
               index = c("country_code", "year"),
               data = fd_data)
...
```{r}
# Contemporaneous Effect: SDG ~ SPI + DI
fe_sdg_spi <- plm(formula = sdg_overall ~ spi_comp + di_score + log_gdppc + income_level_recoded,
  index = c("country_code", "year"),
  data = panel_data,
  model = "within", #FE
  effect = "twoways"
...
# Adding Lag1: SPI ~ DI
fe_sdg_spi_L1 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + di_score + dplyr::lag(di_score, 1) + log_gdppc + income_level_recoded + factor(year), 
               model = "within", 
               index = c("country_code", "year"),
               data = panel_data, 
               effect = "twoways")
...
# Adding Lag2: SPI ~ DI
fe_sdg_spi_L2 <- plm(formula = sdg_overall ~ spi_comp + dplyr::lag(spi_comp, 1) + dplyr::lag(spi_comp, 2) + di_score + dplyr::lag(di_score, 1) + dplyr::lag(di_score, 2) + log_gdppc + income_level_recoded + factor(year), 
               model = "within", 
               index = c("country_code", "year"),
               data = panel_data, 
               effect = "twoways")
