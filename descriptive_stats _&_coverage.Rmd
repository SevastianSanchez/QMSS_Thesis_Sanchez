...
## 3.2 Frequency Table of Country Regime Change Status 
```{r}
# Load panel data with regime change indicators
source("Comp2_panel_wrangling.R")

# Table of regime change status for each country
country_list <- panel_data %>%
  select(country_code, country_name, autocratized, democratized, stable, has_aut_ep, aut_ep, has_dem_ep, dem_ep, has_neither) %>%
  distinct() %>%
