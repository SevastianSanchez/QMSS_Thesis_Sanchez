# Improved regime change identification for discrete event study analysis
...
eiu_identify_regime_changes <- function(data) {
  ...
  result <- data %>%
    ...
    mutate(
      ...
      #------------------------------------------------------------
      # 1. IDENTIFY FUTURE CHANGES (for event study pre-periods)
      #------------------------------------------------------------
      ...
      # Identify pre-democratization years (positive changes)
      ...
      # Combined pre-change indicator for any significant change
