...
## TEST X: FD Mediation Analysis (no lags), SPI ~ DI & SDG ~ SPI 
```{r}
...
## TEST X: FE Mediation Analysis (no lags), SPI ~ DI & SDG ~ SPI 
```{r}
###### SPI ~ DI ######
fe_spi_di <- plm(
...
summary(fe_spi_di)
coeftest(fe_spi_di, vcov = vcovDC(fe_spi_di)) # Robust SEs
...
summary(fe_sdg_spi)
coeftest(fe_sdg_spi, vcov = vcovDC(fe_sdg_spi)) # Robust SEs
...
```
This analysis does not test for autocorrelation using the Durban Watson approach because the approach is designed for single time series, not panel models. Instead, this study adopts the Breusch-Godfrey (BG) approach to detect *high order* autocorrelation in addition to other forms. 
...
#First Difference 
fd_spi_di_lag <- plm(
  spi_comp ~ plm::lag(di_score) + log_gdppc + population,
  data = panel_data,
...
coeftest(fe_sdg_spi, vcov = vcovDC(fe_sdg_spi))
...
# First Difference 
fd_sdg_spi_lag <- plm(
  sdg_overall ~ plm::lag(spi_comp) + di_score + log_gdppc + population,
  data = panel_data,
  index = c("country_code", "year"),
  model = "fd" #FD
)
