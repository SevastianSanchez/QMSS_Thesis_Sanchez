...
```
*Results:*
ACEM: SPI’s indirect effect = 1.0574  units (p < 2e-16***)
ADE: Regime type’s direct effect, excluding SPI = 0.2143  (p = 0.066) [just over 0.05]
Total Effect: = 1.2716 units (p < 2e-16***)
Proportion Mediated: = 0.8307 or 83.07% of total units

`Proportion Mediated` = the percent difference between the total effect (SDG ~ DI) and indirect ACEM effect (+ SPI)

Interpretation: A 1-unit DI increase boosts SDG scores by 1.2716 total units, with 1.0574 units (83.07% of units) transmitted through SPI. The remaining 0.2143 units represents democracy score's effect (e.g., governance reforms unrelated to statistics) on SDG progress. 
...
#ACF tests for serial correlation 
...
```{r}
# SPI ~ DI [SIGNIFICANT]
pgrangertest(spi_comp ~ di_score, order = 1, 
            index = c("country_code", "year"), 
...
### ACF, Includes Lagged Models 
```{r}
acf(resid(fd_spi_di_lag), lag.max = 10)
acf(resid(fd_sdg_spi_lag), lag.max = 10)
acf(resid(fe_spi_di_lag), lag.max = 10)
acf(resid(fe_sdg_spi_lag), lag.max = 10)
```
