...
#read merged_df csv
fd_fe_data <- read_csv("merged_full_final_df.csv")

#selecting 
fd_fe_data <- fd_fe_data %>% 
  dplyr::select(country_code, year, year_fct, sdg_overall, spi_comp, di_score, aut_ep, dem_ep, income_level, income_level_lab, gini, log_gdppc, population) %>% 
  arrange(country_code, year)  # Critical for correct lagging
  
```

# TEST 1: Mediation analysis [REDO RESULTS FOR ROBUST SEs]
...
total_sdg_di_ols <- lm(sdg_overall ~ di_score + log_gdppc + population + gini + year_fct, data = fd_fe_data)
summary(total_sdg_di_ols)

#Mediator model: Check if regime type affects SPI
med_spi_ols <- lm(spi_comp ~ di_score + log_gdppc + population + gini + year_fct, data = fd_fe_data)
summary(med_spi_ols)

#outcome model: Check if SPI affects SDG scores while controlling for regime type
output_sdg_ols <- lm(sdg_overall ~ spi_comp + di_score + log_gdppc + population + gini + year_fct, data = fd_fe_data)
...
#plot(sens_med)
```
ACEM: SPI’s indirect effect = 0.62183 units
ADE: Regime type’s direct effect, excluding SPI = -0.00535??? units
Total Effect: = 0.61648 units
Proportion Mediated: = 100.890%?? of total units

Interpretation: A 1-unit DI increase boosts SDG scores by 0.61648 total units, with 0.62183 units (~100.89% of units) transmitted through SPI. The remaining -0.00535 units reflect direct DI effects (e.g., governance reforms unrelated to statistics).
...
Because the ADE (the direct effect between di_score on sdg_overall) is not significant, SPI DOES ENTIRELY explain the connection between regime type and sdg status, based on the model. *[DEFINATELY NEEDS CONFIRMATION]*
...
summary(fd_spi_di)
coeftest(fd_spi_di, vcov = vcovHC(fd_spi_di, cluster = "group")) #Robust SEs 
...
summary(fe_spi_di)
coeftest(fe_spi_di, vcov = vcovHC(fe_spi_di, cluster = "group")) # Robust SEs
```
...
summary(fd_sdg_spi)
coeftest(fd_sdg_spi, vcov = vcovHC(fd_sdg_spi, cluster = "group")) # Robust SEs
...
summary(fe_sdg_spi)
coeftest(fe_sdg_spi, vcov = vcovHC(fe_sdg_spi, cluster = "group")) # Robust SEs
```
