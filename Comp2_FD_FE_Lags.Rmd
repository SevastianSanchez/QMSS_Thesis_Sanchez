...
## TEST 1: OLS (Y ~ X), The effect of democracy score on SDG
```{r}
...
## TEST 2: OLS The effect of democracy score on SDG (Y ~ M ~ X)
...
fd_data <- fd_fe_data %>%
  ...
  group_by(country_code) %>%
  # Create first differences for selected variables
  mutate(
...
#### pbg tests for autocorrelation ####
# total effect 
pbgtest(fe_sdg_di_lag, order = 1) # AR(1)
pbgtest(fe_sdg_di_lag, order = 2) # AR(2)
# mediator 
...
# ALL MISC OR NO LONGER NEEDED OR TEMORARILY OUT OF THE WAY 
