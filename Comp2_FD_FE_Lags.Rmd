...
**Main controls**
log_gdppc: Log of GDP Per Capita of country 
...
fd_fe_data <- fd_fe_data %>% 
  dplyr::select(country_code, year, year_fct, sdg_overall, spi_comp, di_score, aut_ep, dem_ep, elect_dem, lib_dem, part_dem, delib_dem, egal_dem, academ_free, income_level, income_level_lab, log_gdppc, population) %>% 
...
total_sdg_di_ols <- lm(sdg_overall ~ di_score + log_gdppc + year_fct, data = fd_fe_data)
...
med_spi_ols <- lm(spi_comp ~ di_score + log_gdppc + population + factor(year), data = fd_fe_data)
summary(med_spi_ols)
...
output_sdg_ols <- lm(sdg_overall ~ spi_comp + di_score + log_gdppc + population + factor(year), data = fd_fe_data)
...
#First Difference 
fd_spi_di_lag <- plm(
  spi_comp ~ lag(di_score, order = 1) + log_gdppc + population,
  data = panel_data,
