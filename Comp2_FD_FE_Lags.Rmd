...
## TEST X: The effect of democracy score on SPI (M ~ X)
...
panel_data <- pdata.frame(
  fd_fe_data, 
  index = c("country_code", "year")  #country_code = subject ; # year = time (we need both)
)
...
#summary(fd_sdg_spi)
summary(fd_sdg_spi, vcov = vcovHC(fd_sdg_spi, cluster = "group"))
# Country-level custard Robust SEs
...
###### SPI ~ DI ######
fe_spi_di <- plm(
  spi_comp ~ di_score + log_gdppc + factor(year),
  data = panel_data,
  model = "within" #FE
)
...
#First Difference 
fd_spi_di_lag <- plm(
  spi_comp ~ lag(di_score, order = 1) + log_gdppc,
  data = panel_data,
  index = c("country_code", "year"),
  model = "fd" #FD
)
...
# First Difference 
fd_sdg_spi_lag <- plm(
  sdg_overall ~ lag(spi_comp, 1) + di_score + log_gdppc,
  data = panel_data,
  index = c("country_code", "year"),
  model = "fd" #FD
)
...
# Fixed Effects 
fe_spi_di_lag <- plm(
  spi_comp ~ plm::lag(di_score) + log_gdppc + factor(year),
  data = panel_data,
  index = c("country_code", "year"),
  model = "within" #FE
)
...
# Fixed Effects 
fe_sdg_spi_lag <- plm(
  sdg_overall ~ plm::lag(spi_comp) + di_score + log_gdppc + factor(year),
  data = panel_data,
  index = c("country_code", "year"),
  model = "within" #FE
)
